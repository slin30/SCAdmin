#' Get configuration summary for one or more report suite
#' 
...
#' 
#' @param rsids (required) A \code{character} vector of report suite IDs. Lists will be collapsed to atomic
#' @param locale (required) A \code{character} vector of length 1 denoting the locale. Defaults to \code{en_US}.
...
#' }
Get_Settings <- function(rsids, locale = "en_US") {
  
  if(!is.character(locale) || length(locale) > 1L) {
    stop("locale must be a length 1 character")
  }
  ...
  body <- list(
    locale = jsonlite::unbox(locale),
    rsid_list = c(rsids)
  )
  
  call <- ApiRequest(jsonlite::toJSON(body), "ReportSuite.GetSettings")
